...
RSpec.describe ApiController, type: :controller do
  ...
  describe 'GET get_schedule' do
    ...
    it 'returns the correct schedule for the day' do
      Schedule.create(stage: 1, area: 2, day_of_month: 3, outages: "02:00 - 04:30, 10:00 - 12:30, 18:00 - 20:30")
    ...
    it 'correctly works out today & tomorrow' do
      Schedule.create(stage: 1, area: 2, day_of_month: today.day, outages: "02:00 - 04:30, 10:00 - 12:30, 18:00 - 20:30")
      Schedule.create(stage: 1, area: 2, day_of_month: tomorrow.day, outages: "18:00 - 20:30")
    ...
    it 'errors out if it cant find the schedule' do
      get :get_schedule, stage: 1, area: 16, date: 'today'
      expect(JSON.parse(response.body)).to eq({"error" => "Schedule not found"})
    end
  end

  describe 'GET get_schedule_v2' do
    let!(:today) { Date.today }
    let!(:tomorrow) { Date.today + 1}
    ...
    it 'returns the correct schedule for the day' do
      ...
      get :get_schedule_v2, stage: 'stage1', area: '2', date: '2014-05-03'
      expect(JSON.parse(response.body)).to eq({
        "area_id" => 2,
        "date" => "2014-05-03",
        "day_of_month" => 3,
    ...
    it 'errors out if missing stage param' do
      get :get_schedule_v2, area: '2', date: '2014-05-03'
      expect(JSON.parse(response.body)).to eq({"error" => "Missing 'stage' parameter"})
    end

    it 'errors out if missing area param' do
      get :get_schedule_v2, stage: 1, date: '2014-05-03'
      expect(JSON.parse(response.body)).to eq({"error" => "Missing 'area' parameter"})
    end

    it 'errors out if missing date param' do
      get :get_schedule_v2, stage: 1, area: '2'
      expect(JSON.parse(response.body)).to eq({"error" => "Missing 'date' parameter"})
    end
    ...
    it 'correctly works out today & tomorrow' do
      ...
      get :get_schedule_v2, stage: 'stage1', area: '2', date: 'today'
      expect(JSON.parse(response.body)).to eq({
        "area_id" => 2,
        "date" => today.to_s,
        "day_of_month" => today.day,
      ...
      get :get_schedule_v2, stage: 'stage1', area: '2', date: 'tomorrow'
      expect(JSON.parse(response.body)).to eq({
        "area_id" => 2,
        "date" => tomorrow.to_s,
        "day_of_month" => tomorrow.day,
    ...
    it 'errors out if it cant parse date' do
      get :get_schedule_v2, stage: 1, area: '2', date: 'unparseable'
      expect(JSON.parse(response.body)).to eq({"error" => "Badly formatted date: 'unparseable'"})
    end

    it 'errors out if it cant parse stage' do
      get :get_schedule_v2, stage: 'unparseable', area: '2', date: 'today'
      expect(JSON.parse(response.body)).to eq({"error" => "Unknown stage: 'unparseable'"})
    end

    it 'errors out if area out of range' do
      get :get_schedule_v2, stage: 1, area: 20, date: 'today'
      expect(JSON.parse(response.body)).to eq({"error" => "Area number not in range 1-16: 20"})
    end
  end
